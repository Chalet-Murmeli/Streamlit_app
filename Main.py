...
if app_mode=='Home':
    ...
    st.image('images/loan_image.jpg')    
    st.markdown('Dataset :')    
    ...
    st.image('images/slider-short-3.jpg')
    ...
    if st.button("Predict"):
                 file_ = open("images/6m-rain.jpg", "rb")
                 contents = file_.read()
                 data_url = base64.b64encode(contents).decode("utf-8")
                 file_.close()
                 file = open("images/green-cola-no.gif", "rb")
                 contents = file.read()
                 data_url_no = base64.b64encode(contents).decode("utf-8")
                 file.close()
#                 loaded_model = pickle.load(open('data/Random_Forest.sav', 'rb'))
